...
class FluidCarouselState extends State<FluidCarousel>
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      onPanUpdate: (details) async => _handlePanUpdate(details, _getSize()),
  ...
  void _handlePanUpdate(DragUpdateDetails details, Size size) async {
    double dx = details.localPosition.dx - _dragOffset.dx;

    if (!(await _isSwipeActive(dx))) {
      return;
    }
  ...
  Future<bool> _isSwipeActive(double dx) async {
    ...
    if (_dragDirection == 0.0 && dx.abs() > 20.0) {
      ...
      // prevent left swipe when at index 0
      if (_index == 0 && _dragDirection == 1.0) {
        return false;
      }
      // if user swiped through the last page
      if (_index == widget.children.length - 1 && _dragDirection == -1.0) {
